...
class CycleGANSemanticModel(BaseModel):
    ...
    def forward(self):
        print("Input real_A size:", self.real_A.size())  # Size of the input image A
        print("Input real_B size:", self.real_B.size())  # Size of the input image B

        self.fake_B = self.netG_A(self.real_A)
        print("Output fake_B size:", self.fake_B.size())  # Size after first generator

        self.rec_A = self.netG_B(self.fake_B)
        print("Output rec_A size:", self.rec_A.size())  # Size after cycle back to A

        self.fake_A = self.netG_B(self.real_B)
        print("Output fake_A size:", self.fake_A.size())  # Size after second generator

        self.rec_B = self.netG_A(self.fake_A)
        print("Output rec_B size:", self.rec_B.size())  # Size after cycle back to B

        if self.isTrain:
            # Forward all four images through classifier
            # Keep predictions from fake images only
            self.pred_real_A = self.netCLS(self.real_A)
            print("Pred real_A size:", self.pred_real_A.size())  # Classifier output size for real_A

            _, self.gt_pred_A = self.pred_real_A.max(1)
            pred_real_B = self.netCLS(self.real_B)
            print("Pred real_B size:", pred_real_B.size())  # Classifier output size for real_B

            _, self.gt_pred_B = pred_real_B.max(1)
            self.pred_fake_A = self.netCLS(self.fake_A)
            print("Pred fake_A size:", self.pred_fake_A.size())  # Classifier output size for fake_A

            self.pred_fake_B = self.netCLS(self.fake_B)
            print("Pred fake_B size:", self.pred_fake_B.size())  # Classifier output size for fake_B
    ...
    def backward_CLS(self):
        ...
        print("Output pred_A size in backward_CLS:", pred_A.size())

        self.loss_CLS = self.criterionCLS(pred_A, label_A)
        print("Loss CLS size in backward_CLS:", self.loss_CLS.size())
    ...
    #         # Check if input_A_label and input_B_label are set and calculate semantic losses
    #     if hasattr(self, 'input_A_label') and hasattr(self, 'input_B_label'):
    #         self.loss_sem_AB = self.criterionCLS(self.pred_fake_B, self.input_A_label)
    #         self.loss_sem_BA = self.criterionCLS(self.pred_fake_A, self.input_B_label)
    #     else:
    #         # Handle the scenario when labels are not available
    #         self.loss_sem_AB = 0
    #         self.loss_sem_BA = 0

    #     # Use semantic loss only when classifier has reasonably low loss
    #     if hasattr(self, 'loss_CLS') and self.loss_CLS.detach().item() <= 1.0:
    #         self.loss_G += self.loss_sem_BA + self.loss_sem_AB
